...
export const getNextOrderNo = async () => {
    return new Promise(async (resolve, reject) => {
        Order.find().count().then((count) => {
            if (count > 0) {
                resolve("SKNOD" + (Number(1000001) + count));
            } else {
                resolve("SKNOD" + Number(1000001));
            }
        });
    });
}
...
export const generateHairKit = async (customer, problem) => {
    ...
    let doctorData = await DoctorDetail.find({ isActive: true, isDeleted: false, isHod: false }).distinct("_id");
...
export const generateSkinKit = async (customer, problem) => {
    ...
    if (userData?.gender?.toLowerCase() == 'female') {
        ...
        getKitBySymptomIds = await kit.findOne({ isActive: true, symtom: { $in: symptomIds }, name: { $not: new RegExp('Pregnancy') } });
    }
    if (getKitBySymptomIds == null) {
...
export const skinKitGenerateWithApi = async (basicApiRes, advanceApiRes, userId) => {
    ...
    if (userData?.gender?.toLowerCase() == 'female') {
        ...
        kits = await kit.find({ isActive: true, symtom: { $in: symptoms }, name: { $not: new RegExp('Pregnancy') } });
    }
    kits = kits.map((kData) => {
        let countMatch = 0;
...
export const getCustomFieldByCustomerId = async (customerId, key) => {
    let customer = await User.findById(customerId);
    return customer?.[key];
}
